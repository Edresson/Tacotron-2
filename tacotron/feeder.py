...
class Feeder:
	...
	def __init__(self, coordinator, metadata_filename, hparams):
		...
		with open(os.path.join(os.path.dirname(metadata_filename),'test.txt'), encoding='utf-8') as f:
			self._test_metadata = [line.strip().split('|') for line in f]
			hours = sum([int(x[4]) for x in self._test_metadata]) * hparams.frame_period / (3600 * 1000)
			log('Loaded test metadata for {} examples ({:.2f} hours)'.format(len(self._test_metadata), hours))
